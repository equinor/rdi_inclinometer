...
class OutputManager(object):
    working_folder = "c:\\Appl\\temp\\"
    working_filename = "inclinometer_data"
    working_excel_sheet = "data"
    
    def __init__(self):
        self.filename_raw_output = self.working_folder + self.working_filename + "_" + time.strftime("%Y%m%d", time.localtime()) + "_RAW.txt"
        self.filename_output = self.working_folder + self.working_filename + "_" + time.strftime("%Y%m%d", time.localtime()) + ".txt"
        self.excel_manager = excel_manager.ExcelManager(self.working_folder, self.working_filename, self.working_excel_sheet)
